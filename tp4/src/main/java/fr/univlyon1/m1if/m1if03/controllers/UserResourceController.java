...
public class UserResourceController extends HttpServlet {
     ...
     * Renvoie un code 201 (Created) en cas de création d'un utilisateur, ou une
     * erreur HTTP appropriée sinon.
     *
     * @param request  Une requête de création, contenant un body avec un login, un
     *                 password et un nom
     * @param response Une réponse vide, avec uniquement un code de réponse et
     *                 éventuellement un header <code>Location</code>
        ...
        if (url.length == 1) {// Création d'un utilisateur
            ...
            try {
                ...
                response.sendError(HttpServletResponse.SC_CONFLICT,
                        "Le login " + requestDto.getLogin() + " n'est plus disponible.");
            }
        } else {
     ...
     * </ul>
     * Renvoie un code de réponse 200 (OK) en cas de représentation, 302 (Found) en
     * cas de redirection, sinon une erreur HTTP appropriée.
     *
     * @param request  Une requête vide
     * @param response Une réponse contenant :
     *                 <ul>
     *                 <li>la liste des liens vers les instances de
     *                 <code>User</code> existantes</li>
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (url.length == 1) { // Renvoie la liste de tous les utilisateurs
            ...
            request.setAttribute("view", "users/users");
        ...
        try {
            ...
            switch (url.length) {
                case 2 -> { // Renvoie un DTO d'utilisateur (avec les infos nécessaires pour pouvoir le
                            // templater dans la vue)
                    request.setAttribute("model", ((boolean) request.getAttribute("authorizedUser")) ? userDto : new UserResponseDto(userDto.getLogin(), 
                    userDto.getName(), null));
                    request.setAttribute("view", "users/user");
                    response.setStatus(HttpServletResponse.SC_OK);
                ...
                case 3 -> { // Renvoie une propriété d'un utilisateur
                    switch (url[2]) {
                        case "name" -> {
                            ...
                            request.setAttribute("view", "users/userProperty");
                            response.setStatus(HttpServletResponse.SC_OK);

                        }
                        case "assignedTodos" -> {
                            request.setAttribute("model",
                                    new UserResponseDto(userDto.getLogin(), null, userDto.getAssignedTodos()));
                            request.setAttribute("view", "users/userProperty");
                        response.setStatus(HttpServletResponse.SC_OK);

                        }
                        default ->
                            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Propriété demandée erronée.");
                    }
                }
     ...
     * </ul>
     * Renvoie un code de statut 204 (No Content) en cas de succès ou une erreur
     * HTTP appropriée sinon.
     *
     * @param request  Une requête dont l'URL est de la forme
     *                 <code>/users/{login}</code>, et contenant :
     *                 <ul>
     ...
     * En clair : appelle simplement l'opération de suppression de
     * l'utilisateur.<br>
     * Renvoie un code 204 (No Content) si succès ou une erreur HTTP appropriée
     * sinon.
     *
     * @param request  Une requête dont l'URL est de la forme
    ...
    /**
     * Nested class qui réalise les opérations "simples" (CRUD) de gestion des
     * ressources de type <code>User</code>.
     * Son utilité est surtout de prendre en charge les différentes exceptions qui
     * peuvent être levées par le DAO.
     *
     * @author Lionel Médini
     */
    private static class UserResource {
         ...
         * @throws IllegalArgumentException  Si le login est null ou vide ou si le
         *                                   password est null
         * @throws NameAlreadyBoundException Si le login existe déjà
         * @throws ForbiddenLoginException   Si le login est "login" ou "logout" (ce qui
         *                                   mènerait à un conflit d'URLs)
         */
        public void create(@NotNull String login, @NotNull String password, String name)
            ...
            // Protection contre les valeurs de login qui poseraient problème au niveau des
            // URLs
         ...
         * @throws NameNotFoundException    Si le login ne correspond à aucune entrée
         *                                  dans le DAO
         * @throws InvalidNameException     Ne doit pas arriver car les clés du DAO user
         *                                  sont des strings
         */
        public User readOne(@NotNull String login)
         ...
         * @throws InvalidNameException     Ne doit pas arriver car les clés du DAO user
         *                                  sont des strings
         * @throws NameNotFoundException    Si le login ne correspond pas à un
         *                                  utilisateur existant
         */
        public void update(@NotNull String login, String password, String name)
         ...
         *                                  sont des strings
         */
        public void delete(@NotNull String login)
                throws IllegalArgumentException, NameNotFoundException, InvalidNameException {
            if (login == null || login.isEmpty()) {
                throw new IllegalArgumentException("Le login ne doit pas être null ou vide.");
            }
