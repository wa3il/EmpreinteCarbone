...
public class UserBusinessController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        if (request.getRequestURI().endsWith("login")) {
            ...
            if (requestDto.getLogin() != null && !requestDto.getLogin().isEmpty()) {
                try {
                    User user = userBusiness.userLogin(requestDto.getLogin(), requestDto.getPassword());
                    if (user != null) {
                        String token = TodosM1if03JwtHelper.generateToken(user.getLogin(), (userMapper.toDto(user)).getAssignedTodos(), request);
            ...
            userBusiness.userLogout(response);
            response.setStatus(HttpServletResponse.SC_NO_CONTENT);
        } else {
    ...
    private static class UserBusiness {
         ...
         */
        public User userLogin(@NotNull String login, String password)
         ...
         *
         * @param response  la requête qui contient la session à invalider
         */
